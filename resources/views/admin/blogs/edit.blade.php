            ...
            <form action="{{ route('blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="flex items-center sm:col-span-2 justify-center w-full">
                    <label for="dropzone-file"
                        ...
                        <div id="dropzone-preview"
                            ...
                            @if($blog->thumbnail)
                                <img src="{{ asset($blog->thumbnail) }}" alt="Preview" class="object-contain w-full h-full">
                            @else
                ...
                <div>
                    ...
                    dark:bg-gray-700 dark:border-gray-600 dark:text-white p-2.5">
                        @foreach ($categories as $cate)
                            <option value="{{ $cate->id }}" {{ $cate->id == $blog->category_id ? 'selected' : '' }}>
                                {{ $cate->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    dark:bg-gray-700 dark:border-gray-600 dark:text-white p-3"
                        placeholder="Nhập nội dung..." required>{{ old('content', $blog->content) }}</textarea>
                </div>
